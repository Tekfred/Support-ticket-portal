  ...
  >
    ...
    <div class="h-20 flex items-center justify-between px-5 border-b border-slate-800">
      <div class="flex items-center gap-3 overflow-hidden">
        <div class="h-10 w-10 shrink-0 bg-indigo-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-indigo-500/20">
        ...
        <div v-if="!isCollapsed" class="flex flex-col">
          ...
          <span class="text-indigo-400 font-mono text-[10px] tracking-widest font-semibold uppercase">
            Booking Support
          </span>
        </div>
      </div>
    ...
    <div class="flex-1 py-8 px-4 space-y-8 overflow-y-auto">
      <div>
        <p v-if="!isCollapsed" class="px-3 text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-3">
          Support Views
        </p>
          ...
          >
            <div class="flex items-center gap-3">
              <component 
                ...
                :class="[activeTab === item.id ? 'text-white' : 'text-slate-400 group-hover:text-indigo-400']"
              />
              <div v-if="!isCollapsed" class="text-left">
                <p class="text-sm font-medium leading-none">
                  {{ item.label }}
                </p>
                <p class="text-[10px] mt-1 font-light" :class="[activeTab === item.id ? 'text-indigo-200' : 'text-slate-500']">
                  {{ item.description }}
                </p>
              </div>
            </div>
      ...
      <div v-if="!isCollapsed" class="p-4 rounded-xl bg-slate-800/40 border border-slate-800/50">
        <div class="flex items-center gap-2 mb-2">
          ...
          <span class="text-[11px] font-semibold text-slate-300 font-display">Priority Grounding</span>
        </div>
        <p class="text-[11px] text-slate-500 leading-relaxed font-sans">
          Dynamic workload balancers are operational. Ticket escalations are synchronized with active storage depots.
        </p>
      </div>
    </div>
    ...
    <div class="p-4 border-t border-slate-800 bg-slate-950/40">
      <div class="flex items-center gap-3" :class="[isCollapsed ? 'justify-center' : 'justify-between']">
        <div class="flex items-center gap-3 overflow-hidden">
          <div class="h-10 w-10 rounded-xl bg-gradient-to-tr from-violet-600 to-indigo-500 flex items-center justify-center text-white font-bold select-none text-sm shrink-0 shadow-lg shadow-indigo-600/20 border border-indigo-400/20 animate-none">
            MM
          </div>
          <div v-if="!isCollapsed" class="text-left overflow-hidden">
            ...
            <p class="text-xs text-slate-500 truncate font-mono mt-1">
              ansahaudi86@gmail.com
            </p>
          </div>
        </div>
        <div v-if="!isCollapsed" class="h-2 w-2 rounded-full bg-emerald-500 ring-4 ring-emerald-500/10" title="Online" />
      </div>
    </div>
  </aside>
</template>
