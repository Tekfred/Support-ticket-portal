...
<template>
  <div>
    <div v-if="tickets.length === 0" class="h-96 rounded-2xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center p-8 text-center bg-white dark:bg-slate-900 dark:border-slate-700 transition-colors">
      <div class="text-slate-300 mb-3 dark:text-slate-600"><Clock class="h-12 w-12" /></div>
      <h3 class="text-lg font-bold text-slate-800 dark:text-slate-200">No support tickets found</h3>
      <p class="text-slate-500 max-w-sm mt-1 text-sm dark:text-slate-400">There are no unassigned tickets that match the filter criteria. Add a new ticket or adjust your filters.</p>
    </div>

    <div v-else-if="viewMode === 'grid'" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6" id="tickets-gird">
      <div v-for="ticket in paginatedTickets" :key="ticket.id" class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 dark:hover:shadow-slate-950/50 transition-all flex flex-col relative group overflow-hidden">
        <div class="flex items-start justify-between mb-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="font-bold text-slate-900 dark:text-slate-100 leading-none group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors font-display">{{ ticket.company }}</h3>
              <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium leading-none">{{ ticket.category }}</p>
            </div>
          </div>
        ...
        <div class="mb-4">
          ...
          <div class="flex items-center justify-between">
            <span class="font-display font-medium text-slate-800 dark:text-slate-200 text-[15px]">{{ ticket.position }}</span>
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 line-clamp-3 mb-5 leading-relaxed bg-slate-50 dark:bg-slate-800/50 p-2.5 rounded-lg border border-slate-100 dark:border-slate-700">{{ ticket.description }}</p>

        <div class="mt-auto border-t border-slate-100 dark:border-slate-700 pt-4 flex items-center justify-between">
          <div class="flex flex-col">
            <span class="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-widest leading-none">Applied</span>
            <span class="text-xs font-medium text-slate-600 dark:text-slate-400 mt-1 font-mono flex items-center gap-1"><Clock class="h-3 w-3 text-slate-400 dark:text-slate-500" />{{ ticket.appliedDate }}</span>
          </div>
          ...
          <div class="flex items-center gap-2">
            ...
            <div class="relative">
              ...
              <div v-if="activeActionsId === ticket.id">
                ...
                <div class="absolute right-0 bottom-full mb-2 w-44 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl dark:shadow-slate-950/50 p-1.5 z-20 transition-colors">
    ...
    <div v-else class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden transition-colors" id="tickets-list-layout">
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse font-sans">
          <thead>
            <tr class="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 font-mono text-[10px] tracking-widest uppercase font-semibold">
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-700 text-sm">
            <tr v-for="ticket in paginatedTickets" :key="ticket.id" class="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition-colors group">
              <td class="p-4 pl-6">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p class="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">{{ ticket.category }}</p>
                  </div>
                </div>
              </td>
              <td class="p-4 font-mono font-semibold text-indigo-600 dark:text-indigo-400 text-xs">#{{ ticket.id }}</td>
              <td class="p-4 text-slate-700 dark:text-slate-300 font-medium">{{ ticket.position }}</td>
              <td class="p-4"><span class="text-xs font-semibold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 px-2.5 py-0.5 rounded">{{ ticket.type }}</span></td>
              ...
              <td class="p-4 font-mono text-xs text-slate-500 dark:text-slate-400">{{ ticket.appliedDate }}</td>
